...
- Changes tab (`ConversationTab` value `"changes"`, `src/routes/changes-tab.tsx`): Cursor-style SCM pane — left tree of uncommitted paths from `useUnifiedGetGitChanges`, right pane `FileDiffViewer` for the selection. `navigateToChanges()` opens this tab (not Commits). Commits tab remains commit history. Text files in the Files tab are editable via `EditableSourceView` + `saveWorkspaceTextFile` (Cmd/Ctrl+S or Save); binary / rich markdown previews stay read-only. Agent file-editor writes also enter `agent-review-store` (pending hunks + per-user-message checkpoints). Files shows Accept/Reject per hunk; Restore Checkpoint remains available on that user message. This is write-then-review (Cursor-like), not SDK propose-before-write.

- Collapsible thinking: `ThinkAction`, `ActionEvent.thought`, and LLM extended reasoning (`reasoning_content` / `thinking_blocks` on `ActionEvent`) all render through `CollapsibleThinking` (`src/components/conversation-events/chat/event-message-components/collapsible-thinking.tsx`). Collapsed by default to keep the chat compact — the thinking is often in English regardless of the user's conversation language. Settled labels follow Cursor: measured duration `< 2s` → `THINKING$SETTLED_BRIEF` ("Thought briefly"); `≥ 2s` → `THINKING$SETTLED_DURATION` ("Thought Ns"); no measured duration (historical) → plain `OBSERVATION_MESSAGE$THINK`. Live thinking still uses shimmer `THINKING$TITLE` plus a side elapsed span. Label selection lives in `settled-thinking-label.ts`; `getReasoningContent()` in `event-thought-helpers.ts` prefers `reasoning_content` then Anthropic `thinking_blocks`. Tests: `__tests__/components/conversation-events/chat/event-message-think-action.test.tsx`, `collapsible-thinking-spinner.test.tsx`, `settled-thinking-label.test.ts`.
...
- Electron desktop app (`npm run desktop` for dev / `npm run build:desktop` for the binary) starts the same stack as `dev-with-automation.mjs` but inside an Electron BrowserWindow. Port allocation prefers the defaults from `config/defaults.json` (ingress 8000, agent-server 18000, automation 18001) and falls back to OS-assigned free ports when those are busy; `electron/main.mjs` loads `buildDesktopAppOrigin(config.ingressPort)` rather than a hardcoded `:8000`. The launcher-seeded `default-local` backend also syncs its stored `host` (not only the session API key) to the served origin so a prior `:8000` entry does not keep API calls pointed at a dead port after a fallback launch. Two gotchas live here:
